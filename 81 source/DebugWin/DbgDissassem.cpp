...
AnsiString TDbg::Disassemble(int *Ad)
{
        ...
        Opcode = GetMem(Addr);
        if (Addr>=32768 && !(Opcode&64)) Opcode=0;
        StrAddr = "$"+Hex16(Addr);
        StrCode += Hex8(Opcode);
        Addr++;
