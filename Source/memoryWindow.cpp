        ...
        : TForm(Owner)
{
        ...
        mCharSize = Canvas->TextExtent(String("0"));
...
void RowRenderer::AddressOut(void)
{
        ...
        if (mAddress >= 0)
        {
                TextOut(mCHDC, 0, mY, String::IntToHex(mAddress,4).c_str(), 4);
        }
        else
        {
                TextOut(mCHDC, 0, mY, String::IntToHex(65536 + mAddress,4).c_str(), 4);
        }
}
...
void DecimalRowRenderer::RenderRow(void)
{
        ...
        for (int x = 0; x < mDisplayCellsPerRow; ++x)
        {
                ...
                if ((mAddress >= 0x0000) && (mAddress <= 0xFFFF))
                {
                        ...
                        int val = getbyte(mAddress);
                        String value = Format("%.3d", ARRAYOFCONST((val)));
...
//        TextOut(mCHDC, mLMargin + 2 * mKern - charSize.cy, 0, "+", 1);
        ...
        for (int x = 0; x < mDisplayCellsPerRow; ++x)
        {
		String heading = String::IntToHex(x, 2);
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
        ...
        if (gotAddress)
        {
                ...
                JumpTo1->Caption += " $" + String::IntToHex(address,4);
                JumpTo1->Tag = address;
         }
        else
        {
